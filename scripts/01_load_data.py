...
pe_pathway = "data/GSE25906_series_matrix.txt"

pe_data = pd.read_csv(pe_pathway, sep="\t", skiprows=67, skipfooter=1, engine="python")
print ("Loaded new Preeclampsia data:")
...
with open("data/GSE25906_series_matrix.txt") as file:
...
diag_line = None
for line in meta_lines:
    if line.startswith("!Sample_characteristics_ch1") and ('"classification:') in line:
        diag_line = line

print(diag_line[0:100])

matches = [m for m in meta_lines
           if m.startswith("!Sample_characteristics_ch1") and ('"classification:' in m) ]
print(len(matches))
diag_line = matches[0]
...
print("Saved the pe_data and diagnosis data as CSV", pe_data.shape, diagnosis.shape)